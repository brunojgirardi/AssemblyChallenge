...
public class MainViewerTesting {
	...
	public void main() {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MainViewerTesting window = new MainViewerTesting();
	...
	private void initialize() {
		...
		frmMainwindow.setIconImage(Toolkit.getDefaultToolkit()
				.getImage(MainViewerTesting.class.getResource("/javax/swing/plaf/metal/icons/ocean/hardDrive.gif")));
		frmMainwindow.setTitle("Conference Managment");
		...
		btnGenerate.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					if (dir.trim().equals(null)) {
						executeFile(dir);
					} else {
						JOptionPane.showMessageDialog(null, "There is not file attached. Try again.");
					}
				} catch (Exception ee) {
